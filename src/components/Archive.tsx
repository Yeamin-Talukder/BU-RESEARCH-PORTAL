...
import { Calendar, ChevronRight, Layers, FileText } from 'lucide-react';
...
const Archive: React.FC = () => {
    ...
    const fetchVolumes = async () => {
        try {
            ...
            setVolumes(data);
            if (data.length > 0) {
                // Auto select newest volume?
                // setSelectedVolume(data[0]._id);
            }
        } catch (error) {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center mb-16">
                    ...
                    <p className="text-lg text-slate-600 max-w-2xl mx-auto font-body">
                        Browse our publication history by Year (Volume) and Issue.
                    </p>
                </div>
                                    ...
                                    <ul className="divide-y divide-slate-100">
                                        {volumes.map((vol) => (
                                            <li key={vol._id}>
                                                ...
                                                {/* Issues Nested List (Mobile/Sidebar logic if preferred, keeping simple for now) */}
                                            </li>
                                        ))}
                                    </ul>
                                )}
                    ...
                    <div className="lg:col-span-9 space-y-8">
                        {!selectedVolume && (
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
                                ...
                                <p className="text-slate-500">Choose a year/volume from the sidebar to view published issues.</p>
                            </div>
                        )}
                        ...
                        {selectedIssue && (
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                    ...
                                    <div className="bg-white px-3 py-1 rounded-md border border-slate-200 text-sm font-medium text-slate-600 shadow-sm">
                                                ...
                                                <div className="flex gap-4">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="flex flex-col sm:flex-row sm:items-center text-sm text-slate-600 gap-y-2 gap-x-6">
                                                             ...
                                                             {/* Add page numbers or other metadata here if available */}
                                                        </div>
                                                        
                                                        <div className="mt-4 pt-4 border-t border-dashed border-slate-100 flex gap-3">
                                                            <button 
                                                                onClick={() => navigate(`/paper/${paper._id}`)}
                                                                className="text-xs font-bold text-blue-600 hover:text-blue-800 uppercase tracking-wider flex items-center gap-1"
                                                            >
